...
3. `fly secrets set CHIVAGO_EVENT_TOKEN=<32 random characters> CHIVAGO_REGISTRATIONS_PER_HOUR=500`
4. `python scripts/event-qr.py https://chivago.fly.dev --token <the same> --out qr/`
   and print: the room code at A3, the five place codes at A5. Do not commit
   the images.
5. Open `https://chivago.fly.dev/console/stories` on the stage phone, sign in
   as the moderator, leave it open. Open `https://chivago.fly.dev/board/ku-sriracha`
   on the projector laptop.
6. The team's own stories: sign in as the team on the campus, open the door
